...
func (s *DeviceService) ListByStatus(ctx context.Context, status string, branchID *uint, branchIDs []uint, pq models.PaginationQuery) (*models.PaginatedResponse, error) {
	items, total, err := s.repo.FindByStatus(ctx, status, branchID, branchIDs, pq)
	if err != nil {
		return nil, appErrors.ErrInternal
	}
	return &models.PaginatedResponse{
